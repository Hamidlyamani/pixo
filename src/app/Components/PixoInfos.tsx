...
import logo from "/public/imgs/pixo.webp"
...
export default function PixoInfos() {
  ...
  return (
    <>
      <div onClick={() => setOpen(!open)} className="flex gap-1 absolute right-4 bottom-2  w-fit !items-center justify-center p-4 cursor-pointer " title="About us" >
        <div><Image src={logo} alt="pixo " className="w-[35px]" width={35} /></div>
        <div><h2 className={`text-blue-400 text-xl font-black mt-2 ${sourGummy.className} `} >Pixo</h2></div>
      </div>
      {open && (
        <div className="absolute bottom-24 right-4 md:right-6 w-11/12 md:w-80 p-5 rounded-lg bg-white border border-gray-200 shadow-lg animate-slide-up">
